...
//=============================================
{
	...
	for ( var i = 0; i < matrix[0].length-1; i++) {
		html += "<td class='dimA'>"+UICom.structure["ui"][matrix[0][i+1]].getLabel()+"</td>";
	}
	html += "</tr>";
	//-------------------------------
	for ( var j = 1; j < matrix.length; j++) {
		...
		for ( var i = 0; i < matrix[0].length; i++) {
			if (i>0){
				...
				html += "<td class='dimB'>"+UICom.structure["ui"][matrix[j][i]].getLabel()+"</td>";
			}
		}
		html += "</tr>";
	}
...
//	html += "</a>";
	...
	var inline = false;
	UIFactory['Node'].displayStandard(UICom.structure['tree'][uuid],'window-page',100,lang,true,inline);
	//------------------ footer ---------------
